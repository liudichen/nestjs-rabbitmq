...
import { getHandlerForLegacyBehavior, MessageHandlerErrorBehavior } from "./errorBehaviors";
...
const defaultConfig = {
  ...
  defaultRpcErrorHandler: getHandlerForLegacyBehavior(MessageHandlerErrorBehavior.REQUEUE),
...
export class AmqpConnection {
  ...
  private async initCore(): Promise<void> {
    ...
    this._managedConnection.on("disconnect", ({ err }) => {
      this.logger.error(`Disconnected from RabbitMQ broker (${this.config.name})`, err?.stack);
    });
    ...
    await Promise.all([
      Object.keys(this.config.channels).map(async (channelName) => {
        ...
        if (defaultChannel.name === AmqpConnection.name && config.default) {
          ...
          defaultChannel.config.prefetchCount = config.prefetchCount || this.config.prefetchCount;
          return;
        }
  ...
  private async setupInitChannel(channel: ConfirmChannel, name: string, config: RabbitMQChannelConfig): Promise<void> {
  ...
  private async setupQueuesWithBindings(channel: ConfirmChannel, queues: RabbitMQQueueConfig[]) {
    for (const configuredQueue of queues) {
      const { createQueueIfNotExists = true } = configuredQueue;

      if (createQueueIfNotExists) {
        this.setupQueue(
          { ...configuredQueue, queue: configuredQueue.name, queueOptions: configuredQueue.options },
          channel
        );
      }
  ...
  private async initDirectReplyQueue(channel: ConfirmChannel) {
    ...
    await channel.consume(
      ...
      async (msg) => {
        ...
        const message = msg.content.length > 0 ? this.config.deserializer(msg.content, msg) : undefined;
        ...
        try {
          ...
          if (isNull(msg)) {
            ...
            const errorHandler =
              ...
              getHandlerForLegacyBehavior(msgOptions.errorBehavior || this.config.defaultSubscribeErrorBehavior);
      ...
      try {
        ...
        if (msg == null) {
          ...
          const errorHandler =
            ...
            getHandlerForLegacyBehavior(rpcOptions.errorBehavior || this.config.defaultSubscribeErrorBehavior);
  ...
  private handleMessage<T, U>(
    handler: (msg: T | undefined, rawMessage?: ConsumeMessage, headers?: any) => Promise<U>,
    msg: ConsumeMessage,
  ...
  private async setupQueue(subscriptionOptions: MessageHandlerOptions, channel: ConfirmChannel): Promise<string> {
    const {
      exchange,
      routingKey,
    ...
    if (exchange && routingKeys) {
      await Promise.all(
        routingKeys.map((routingKey) => {
          if (routingKey != null) {
            channel.bindQueue(actualQueue as string, exchange, routingKey, bindQueueArguments);
          }
        })
      );
    }
  ...
  private setupManagedChannel(name: string, config: RabbitMQChannelConfig) {
    ...
    channel.on("error", (err, { name }) =>
      this.logger.log(`Failed to setup a RabbitMQ channel - name: ${name} / error: ${err.message} ${err.stack}`)
    );
  ...
  public async resumeConsumer<T, U>(consumerTag: ConsumerTag): Promise<ConsumerTag | null> {
    ...
    if (consumer.type === "rpc") {
      newConsumerTag = await this.setupRpcChannel<T, U>(consumer.handler, consumer.rpcOptions, consumer.channel);
    } else {
      newConsumerTag = await this.setupSubscriberChannel<T>(consumer.handler, consumer.msgOptions, consumer.channel);
    }
